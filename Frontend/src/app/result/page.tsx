        ...
        try {
          ...
          if (!response.ok) {
            ...
            try {
              ...
              errMsg = errorData.detail || "Gagal menyimpan riwayat";
            } catch {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      if (!cvText && !analysis) {
        ...
        try {
          const response = await fetch("/history/me", {
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${token}`,
            },
    ...
    <div className="flex h-screen">
      ...
      <main className="flex-1 bg-gradient-to-br from-green-100 via-white to-blue-100 p-8 overflow-auto">
        <div className="max-w-4xl mx-auto mt-10 bg-white p-8 rounded-2xl shadow-lg border-2 border-blue-100">
          <Link href="/upload">
            <Button
              className="mb-6 bg-teal-600 text-white hover:bg-teal-700"
              variant={undefined}
              size={undefined}
            >
              Kembali ke Upload
          ...
          {!analysis ? (
            <p className="text-gray-700">
              Tidak ada hasil analisis yang ditemukan. Silakan unggah CV
              terlebih dahulu.
            </p>
          ) : (
